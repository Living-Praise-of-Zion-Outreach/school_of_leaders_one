...
import 'package:school_of_leaders_one/models/modelBibleVerse.dart';
import 'package:school_of_leaders_one/widgets/DailyVerseChapterGenerator.dart';
...
class DailyVerseDetails extends StatefulWidget {
  const DailyVerseDetails({super.key});
  @override State<DailyVerseDetails> createState() => _DailyVerseDetailsState();
}
class _DailyVerseDetailsState extends State<DailyVerseDetails> {
            ...
            child: Column(
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    ElevatedButton(onPressed: (){copyVerseClipboard();},style: ElevatedButton.styleFrom(backgroundColor: myThemes.getColor(colorTheme)), child: Text('Copy Text')),
                    ElevatedButton(onPressed: (){sendNotes();},style: ElevatedButton.styleFrom(backgroundColor: myThemes.getColor(colorTheme)), child: Text('Send To Notes'))
                  ],
                ),
              ],
            ),
  ...
  //  print('my_list_data->' + my_list_data.toString());

    modelDailyVerse BibleReadCheckerList = await sqlHelper.getDailyVerseByID(dailyVerseID);
    late List<String> readBibleChecker = BibleReadCheckerList.status.split(',');

   // print('BibleReadCheckerList->' +BibleReadCheckerList.toString());

    for(int a=0;a<my_list_data.length;a++){

      List<String> perBible = my_list_data[a].split("-");
    //  print("aha0->"+perBible[0]);
    ...
    //  print("bibleBOok->"+bibleBook.long_name);
